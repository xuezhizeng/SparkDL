...
import org.apache.spark.ml.image.core.{BGRImage, ReadImageUtil}
import org.opencv.imgcodecs.Imgcodecs
import org.scalatest.{BeforeAndAfterAll, FlatSpec, Matchers}

class ReadImageSpec extends FlatSpec with Matchers with BeforeAndAfterAll{
...
//  }
  ...
  "read as bytes" should "get same result as ImageIO" in {
    ...
    val (bytes2, h2, w2, c2) = ReadImageSpec.getBytesFromStream(Paths.get(resource.toURI).toFile)
  ...
  "read as bytes" should "get same result as BigDL ImageIO with smallSideSize" in {
    ...
    // ImageIO and rescale
    val bytes2 = BGRImage.readImage(Paths.get(resource.toURI), 256).drop(8)
